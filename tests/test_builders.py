"""Testes para os builders de dados (seed_loja e seed_biblioteca)."""
...
# --- Loja: build_static_entities ---


def test_build_static_entities_returns_three_lists() -> None:
    """build_static_entities devolve (suppliers, products, clients)."""
...
def test_build_static_entities_suppliers_count_and_first() -> None:
    """Fornecedores: 3 entidades, primeiro é Nike."""
...
# --- Biblioteca: build_autores, build_livros, build_leitores, build_emprestimos ---


def test_build_autores_count_and_first() -> None:
    """Autores: 5, primeiro é José Saramago."""
...
def test_build_livros_count_and_first() -> None:
    """Livros: 10, primeiro é Memorial do Convento."""
    livros = build_livros()
...
def test_build_emprestimos_returns_list() -> None:
    """build_emprestimos devolve lista de empréstimos."""
    rng = Random(42)
    emprestimos = build_emprestimos(rng)
    assert isinstance(emprestimos, list)
    assert len(emprestimos) >= 10  # fixos + aleatórios
    first = emprestimos[0]
    assert first.id_emprestimo == 1
    assert first.id_livro == 1
    assert first.id_leitor == 1
    assert first.data_devolucao is not None  # primeiro é devolvido
